...
function initTyping() {
  ...
  if (charIndex < characters.length - 1 && timeLeft > 0) {
    ...
    let cpm = Math.round(
      ((charIndex - mistakes) / (maxTime - timeLeft)) * 60
    );
    let words = Math.round((charIndex - mistakes) / 5);
...
function showResults() {
  ...
  typingText.innerHTML += `<br/><br/>Time's up!`;
  let wpm = Math.round(((charIndex - mistakes) / 5 / maxTime) * 60);
  let cpm = Math.round(((charIndex - mistakes) / maxTime) * 60);
  ...
  `;
  saveRecord(wpm, cpm, mistakes);
  displayRecords();
}

function saveRecord(wpm, cpm, mistakes) {
  let pastRecords =
    JSON.parse(localStorage.getItem("typingRecords")) || [];
  pastRecords.push({ wpm, cpm, mistakes });
  localStorage.setItem("typingRecords", JSON.stringify(pastRecords));
}

function displayRecords() {
  let recordsTable = document.getElementById("records-body");
